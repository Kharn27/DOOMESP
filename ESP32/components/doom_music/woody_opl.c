...
// Keep the emulator API distinct from LittleMUS' adlib_write callback.
#define adlib_init woody_opl_init
#define adlib_write woody_opl_write
#define adlib_getsample woody_opl_getsample
#include "woody_opl.h"
...
// One emulator instance is used by the dedicated audio task, so persistent
// work buffers save several kilobytes of task stack without requiring locks.
static Bit32s output_left[BLOCKBUF_SIZE];
#if defined(OPLTYPE_IS_OPL3)
static Bit32s output_right[BLOCKBUF_SIZE];
#endif
static Bit32s vibrato_lut[BLOCKBUF_SIZE];
static Bit32s tremolo_lut[BLOCKBUF_SIZE];
...
void adlib_getsample(Bit16s* sndptr, Bits numsamples) {
	...
	Bit32s* outbufl = output_left;
#if defined(OPLTYPE_IS_OPL3)
	Bit32s* outbufr = output_right;
#endif

	// vibrato/tremolo lookup tables (global, to possibly be used by all operators)
	Bit32s* vib_lut = vibrato_lut;
	Bit32s* trem_lut = tremolo_lut;
